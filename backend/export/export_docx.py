...
def export_A_to_docx(case_id: str):
    ...
    try:
        with open(tmp_path, "rb") as f:
            save_cache(case_id, "report_A_full.docx", f.read())
    finally:
        try:
            os.remove(tmp_path)
        except OSError:
...
def export_B_to_docx(case_id: str):
    ...
    try:
        with open(tmp_path, "rb") as f:
            save_cache(case_id, "report_B_full.docx", f.read())
    finally:
        try:
            os.remove(tmp_path)
        except OSError:
...
def export_C_to_docx(case_id: str):
    ...
    try:
        with open(tmp_path, "rb") as f:
            save_cache(case_id, "report_C_full.docx", f.read())
    finally:
        try:
            os.remove(tmp_path)
        except OSError:
