...
def run_export_case_A(case_id: str, chain: ExportAChain):
    ...
    # 4) 저장
    path = f"cache/{case_id}/export_A_exec_summary.json"
...
def run_export_case_B(
    ...
    *,
    ...
    cache_path = f"cache/{case_id}/export_B_exec_summary.json"
    ...
    chunks = chunk_issues_by_token(issues=sections, max_est_tokens=max_est_tokens)
    ...
    # ✅ reduce 단계는 final_chain
    final_result = final_chain.run(final_inp)

    path = f"cache/{case_id}/export_B_exec_summary.json"
    with open(path, "w", encoding="utf-8") as f:
...
def run_export_case_C(case_id: str, model: str = DEFAULT_MODEL) -> ExportCOutput:
    ...
    # 0) final cache
    final_cache = export_c_final_cache_path(case_id)
    cached_final = _read_json_safely(final_cache)
    ...
    if est <= MAX_SAFE_TOKENS:
        ...
        _write_json_atomic(final_cache, out.model_dump())
        return out

    # 3) chunked mode: DOC-REDUCE -> ISSUE-CHUNK -> FINAL-REDUCE
    print("[ExportC] chunked mode: DOC-REDUCE -> ISSUE-CHUNK -> FINAL-REDUCE")

    # 3-A) doc_digest (cached)
    doc_digest_cache = export_c_doc_digest_cache_path(case_id)
    cached_doc = _read_json_safely(doc_digest_cache)
    ...
    if isinstance(cached_doc, dict) and isinstance(cached_doc.get("doc_digest"), str) and cached_doc["doc_digest"].strip():
        ...
        print(f"[ExportC] doc_digest cache hit: {doc_digest_cache.name}")
    else:
        ...
        _write_json_atomic(doc_digest_cache, {"doc_digest": doc_digest_str})
    ...
    for idx, issue_chunk in enumerate(issue_chunks, start=1):
        ...
        cache_path = export_c_issue_chunk_cache_path(case_id, issue_key)

        cached_issue = _read_json_safely(cache_path)
        if isinstance(cached_issue, dict) and isinstance(cached_issue.get("issue_chunk_summary"), str) and cached_issue["issue_chunk_summary"].strip():
            ...
            print(f"[IssueChunk {idx}/{len(issue_chunks)}] cache hit: {cache_path.name}")
            continue
        ...
        _write_json_atomic(cache_path, {"issue_chunk_summary": s})
        issue_level_strings.append(s)

    # -------------------------------------------------
    # 3-C) FINAL REDUCE (REDUCE, CACHED)
    # -------------------------------------------------
    final_reduce_cache = Path(f"cache/{case_id}/export_C/final_reduce.json")
    cached_reduce = _read_json_safely(final_reduce_cache)

    if (
        isinstance(cached_reduce, dict)
        and isinstance(cached_reduce.get("merged_issue_str"), str)
        and cached_reduce["merged_issue_str"].strip()
    ):
        merged_issue_str: str = cached_reduce["merged_issue_str"]
        print(f"[FinalReduce] cache hit: {final_reduce_cache.name}")
    else:
        print("[FinalReduce] reducing issue summaries")

        merged_issue_str = reduce_texts_until_one(
            texts=issue_level_strings,
            issue_logic_list=[],  # ✅ reduce는 요약물 병합이므로 issue_logic 비움
            model=model,
            budget_tokens=REDUCE_BUDGET,
            label="FinalReduce",
        )

        final_reduce_cache.parent.mkdir(parents=True, exist_ok=True)
        _write_json_atomic(final_reduce_cache, {"merged_issue_str": merged_issue_str})
    ...
    _write_json_atomic(
        export_c_final_cache_path(case_id),
        final_obj,
    )
